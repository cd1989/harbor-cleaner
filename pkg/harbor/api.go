...
const (
	UpdaterScheduleSetting = "updater-schedule"
	...
	APIImageScan           = "/api/repositories/%s/%s/tags/%s/scan"
	...
	APISystemInfo          = "/api/systeminfo"
	APIVolumes             = "/api/systeminfo/volumes"
	APIJobsRep             = "/api/jobs/replication"
	APIClairUpdate         = "/v1/update"
	APIClairSettings       = "/v1/settings"
)
...
func ImageScanPath(project, repo, tag string) string {
	return fmt.Sprintf(APIImageScan, project, repo, tag)
}
...
func APILatestJob(policyID int64) string {
	return fmt.Sprintf("%s?policy_id=%d&page=1&page_size=1", APIJobsRep, policyID)
}

func ClairUpdaterSchedulePath() string {
	return fmt.Sprintf("%s/%s", APIClairSettings, UpdaterScheduleSetting)
}
